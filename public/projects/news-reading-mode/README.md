...
기사 본문이 한국어가 아니라고 판단되면(영어/일본어), 슬라이더 카드 바로 아래에 **"한국어로 번역"** 버튼이 자동으로 나타납니다.
클릭하면 AI가 번역한 한국어 본문으로 바뀌고, 다시 누르면 슬라이더가 가리키던 단계(원문 또는 변환본)로 돌아갑니다.
슬라이더를 조작하면 번역 표시는 자동으로 해제됩니다. (한국어 기사에는 이 버튼이 뜨지 않습니다.)
