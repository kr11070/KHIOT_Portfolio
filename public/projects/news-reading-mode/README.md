...
지원 언론사 기사 페이지에 들어가면 화면 왼쪽에 세로 슬라이더가 뜹니다.
Agile Squad 프로토타입과 같은 UI로, 트랙을 클릭하거나 드래그해서 단계를 고르면 AI가 변환한 본문으로 바뀝니다.

- **원문** (맨 위): 기사 원래 내용 그대로
- **중간 단계** (중간 점): 어려운 용어를 풀어 쓴 "-습니다"체, 세부 정보 유지
- **쉬운말** (맨 아래): 초등학생도 읽을 수 있는 아주 짧고 친근한 문장

원문에 있던 이미지는 원래 있던 위치(문단 흐름 기준 비율 지점)에 그대로 유지됩니다.
한 번 변환한 결과는 브라우저에 저장(캐싱)되어, 같은 기사를 다시 열면 AI 재요청 없이 즉시 표시됩니다.

### 외국어 기사 번역

기사 본문이 한국어가 아니라고 판단되면(영어/일본어), 슬라이더 카드 바로 아래에 **"한국어로 번역"** 버튼이 두 줄짜리 알약 모양으로 자동 표시됩니다.
클릭하면 AI가 번역한 한국어 본문으로 바뀌고 버튼은 **"번역 끄기"**로 바뀝니다.
번역이 켜진 동안 슬라이더를 움직이면 본문은 그 단계(쉬운말/쉬운말)로 바뀌지만, 번역 버튼 자체의 표시는 그대로 유지됩니다 —
**"번역 끄기"를 직접 눌러야만** 번역되지 않은 진짜 원문으로 돌아가고 슬라이더도 "원문" 위치로 리셋됩니다.
(한국어 기사에는 이 버튼이 뜨지 않습니다.)
